...
function HomePage() {
  ...
  const fecth_products = async () => {
    ...
    try {
      ...
      const initialData = apiProducts.map(product => ({
          ...product,
          isLiked: false, // 💡 Business Logic: กำหนดค่าเริ่มต้นให้กับ UI state
      }));
  ...
  if (error) {
    return (
      <div className="p-8 text-center bg-red-100 border-l-4 border-red-500 text-red-700 max-w-lg mx-auto mt-10 shadow-lg rounded-lg">
        ...
        <p className="mt-4 text-xs">โปรดตรวจสอบว่า Backend Server ของคุณรันอยู่ที่พอร์ต 5000</p>
      </div>
    );
  }
  ...
  const filteredProducts = products.filter((product) => product.status === "upcoming...");
  if (products.length === 0 || filteredProducts.length === 0) {
     return <div className="p-8 text-center text-gray-500 mt-10 text-xl font-medium">
  ...
  return (
    ...
    <div className="min-h-screen bg-gray-50 font-sans">
      ...
      <div className="text-center py-16 bg-white shadow-md">
        <h1 className="text-5xl font-extrabold text-gray-900 mb-2">Picture Auction</h1>
        <p className="text-xl text-gray-600 mb-6">The Real-time Digital Art Bidding Platform</p>
        <Link 
          to="/upcoming" 
      ...
      <div className="container mx-auto p-8">
        <h2 className="text-3xl font-bold text-gray-800 mb-8 border-b-2 border-indigo-200 pb-2">Upcoming Auctions ({filteredProducts.length})</h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
          {filteredProducts.map((product) => {
              ...
              const placeholderText = product.title ? product.title.split(' ')[0] : 'Art';
                ...
                >
                  <div className="relative">
                    <img
                      ...
                      src={imageSource}
                      alt={product.title}
                      onError={(e) => { e.target.onerror = null; e.target.src = "https://placehold.co/400x300/e0e0e0/505050?text=Image+Unavailable"; }}
                    />
                    {/* Heart Button */}
                    <button
                  ...
                  <div className="p-4 flex flex-col flex-grow">
                    <p className="text-xl font-semibold text-gray-900 truncate mb-1" title={product.title}>
                        {product.title || "Untitled Artwork"}
                    </p>
                    <p className="text-sm text-gray-500 mb-2">
                        <span className="font-medium text-gray-700">Current Bid:</span> {product.price || 'N/A'}
                    </p>
